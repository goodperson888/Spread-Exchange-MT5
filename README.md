...
Mac 开发环境不再需要手填模拟 Bid/Ask。页面通过本机 `http://127.0.0.1:22346/mcp` 读取 MT5 账户身份、黄金品种、合约规格和实时 Bid/Ask。MCP 适配器仅实现读取命令，不调用下单、改单或平仓工具。当前为实盘时，手动改选 MCP 会自动回到纸面模式；选择实盘则自动切换到 Windows MT5 原生终端。
