...
币安最优 Bid/Ask 优先使用官方 WebSocket `bookTicker` 长连接，断线自动重连并回退 REST。若客户网络无法直连币安，可在页面填写无账号密码的 HTTP/Mixed 代理（例如 `http://127.0.0.1:7890`），REST、WebSocket、账户检查和实盘下单会统一走该代理；留空则直连。MetaTrader5 Python API 没有 tick 回调/WebSocket 接口，因此应用保持单一 MT5 工作进程和终端会话，每个策略周期读取最新 tick，默认周期 250 毫秒。报价最大年龄默认 3000 毫秒。
