...
高频行情以本地采样时间作为唯一标识；同一时间戳和已完成分钟归档点不会被后续数据覆盖。最右侧当前分钟会持续增加新采样，这是实时监控的正常表现。250 毫秒只是策略检查周期，不是成交保证时间。
...
原生 MT5 Python 接口没有 WebSocket 或推送回调。程序保持 MT5 终端工作进程的长连接，在每个策略检查周期读取 `symbol_info_tick` 返回的最新 Bid/Ask，并使用 tick 的 `time_msc` 做新鲜度校验；账户、持仓和 Swap 等快照约每 2 秒刷新一次。`copy_ticks_from` 可用于批量读取历史 tick，但同样是主动读取，不是推送。
