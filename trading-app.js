...
(() => {
  ...
  let chart, timer, busy=false, last=null, resetChartZoom=true;
  ...
  function lock(value) {
    ...
    for(const id of ['trading-connect','trading-reconcile','trading-toggle','trading-close']) {
      ...
      if(value) el(id).classList.add('processing');
      else el(id).classList.remove('processing');
    }
  }
  async function action(work) {
    if(busy)return; lock(true);
    try { await work(); } catch(error) { text('trading-result',error.message,true); }
    finally { lock(false); }
  }
  function groupRows(groups, orders) {
    ...
    const rows=groups.slice().reverse().map(g => {
      ...
      return `<tr><td><strong>#${escape(g.id)}</strong><br><span class="muted">${dateTime(g.opened_ms)}${g.closed_ms?'<br>→ '+dateTime(g.closed_ms):''}</span></td><td>${escape(states[g.status]||g.status)}<br><span class="record-basis">${basis}</span></td><td>${n(g.lots)} 手 / ${n(g.qty)} 盎司<br><span class="muted">币安 ${amount('binance').toFixed(2)} USDT<br>MT5 ${amount('mt5').toFixed(2)} USD</span></td><td>毛收益 ${usd(v.gross)}<br>手续费 -${Number(v.fees||0).toFixed(2)} USD${Number(v.estimated_exit_fee)>0?'<br>预估平仓费 -'+Number(v.estimated_exit_fee).toFixed(2)+' USD':''}</td><td>${carryDetail}</td><td class="${Number(v.net)>=0?'pnl-positive':'pnl-negative'}"><strong>${usd(v.net)}</strong><br><span class="muted">${g.status==='closed'?'平仓净收益':'实时净收益'}</span></td><td>币安 ${n(remaining.binance||0)}<br>MT5 ${n(remaining.mt5||0)}${g.reason?'<br>'+escape(g.reason):''}</td><td>${action}</td></tr>`;
    }).join('');
  ...
  function render(result) {
    ...
    el('trading-mode').textContent=result.connected ? `${result.capabilities?.mode||'paper'} 已连接` : '未连接';
    el('trading-mode').classList.toggle('warning', Boolean(state.alarm||result.last_error));
    const stateText = state.recovery ? '需要对账' : state.enabled ? '自动开仓运行中' : '已暂停新开仓';
    const alarm=state.alarm||result.last_error;
    text('trading-result', `${stateText}${alarm?'：'+alarm:''}\n${result.plan?`每组：MT5 ${result.plan.lots} 手 ↔ 币安 ${result.plan.qty} 盎司。`: '请保存参数后连接。'}`,Boolean(alarm));
    const auto=result.auto_values||{}, details=[];
    if(auto.fx?.value) {
      el('usdt-fx').value=Number(auto.fx.value).toFixed(6);
      details.push(`USDT/USD ${Number(auto.fx.value).toFixed(6)}（币安资产指数）`);
    }
    if(auto.binance_fee?.taker!==undefined) {
      el('binance-fee').value=Number(auto.binance_fee.taker).toFixed(4);
    ...
    if(q) {
      el('quote-summary').textContent=`币安 Bid/Ask ${n(q.binance.bid)} / ${n(q.binance.ask)}（${date(q.binance.time_ms)}） · MT5 Bid/Ask ${n(q.mt5.bid)} / ${n(q.mt5.ask)}（${date(q.mt5.time_ms)}） · 入场价差 ${n(q.entry)} · 退出价差 ${n(q.exit)} USD/盎司`;
  ...
  async function plot() {
    ...
    el('spread-stats').innerHTML=[['当前入场',latest?.entry],['当前退出',latest?.exit],['窗口入场均值',average(entries)],['价差范围',all.length?`${Math.min(...all).toFixed(3)} ～ ${Math.max(...all).toFixed(3)}`:null]].map(([label,value])=>`<div class="stat"><span>${label}</span><strong>${typeof value==='number'?value.toFixed(3):value||'—'}</strong></div>`).join('');
    el('chart-empty').classList.toggle('is-hidden',samples.length>0);
    el('chart-coverage').textContent=samples.length?`本机可成交价差 ${samples.length.toLocaleString('zh-CN')} 个采样 · ${new Date(samples[0].time_ms).toLocaleString()} 至 ${new Date(samples.at(-1).time_ms).toLocaleString()} · 高频记录保留 24 小时，更早按分钟归档保留 30 天。`:'尚无本机历史；连接后开始记录真实 Bid/Ask 可成交价差。';
    ...
    chart.setOption({animation:false,legend:{top:5,type:'scroll'},tooltip:{trigger:'axis',confine:true,valueFormatter:v=>Number.isFinite(v)?v.toFixed(3):'—'},grid:{left:75,right:120,top:70,bottom:80},xAxis:{type:'time'},yAxis:{type:'value',scale:true,name:'价差 USD/盎司'},dataZoom:[{type:'inside',filterMode:'none',start,end},{type:'slider',filterMode:'none',bottom:15,start,end}],series:[
      {id:'entry',name:'入场可成交价差',type:'line',showSymbol:false,sampling:'lttb',connectNulls:false,data:lines.map(x=>[x.time_ms,x.entry]),itemStyle:{color:'#246b92'},lineStyle:{width:2,type:'solid'},markArea:{silent:true,itemStyle:{color:'rgba(36,107,146,.07)'},data:[[{name:'开仓触发区',yAxis:Number(el('entry').value)},{yAxis:'max'}]]},markLine:{symbol:'none',silent:true,data:[{name:'开仓阈值',yAxis:Number(el('entry').value)},...marks],label:{formatter:'{b}: {c}'}}},
      {id:'exit',name:'退出可成交价差',type:'line',showSymbol:false,sampling:'lttb',connectNulls:false,data:lines.map(x=>[x.time_ms,x.exit]),itemStyle:{color:'#247b68'},lineStyle:{width:2,type:'dashed'}},
  ...
  async function status(withChart=true) {
    const r=await request('/api/trading/status'); render(r); if(withChart) await plot();
  }
  async function connect() {
    await request('/api/config', formConfig());
    ...
    el('trading-api-key').value='';el('trading-api-secret').value='';render(r);await plot();
  }
  async function closeOne(group) { render(await request('/api/trading/close',{group,reason:'用户请求平仓'})); }
  el('trading-connect').onclick=()=>action(connect);
  el('trading-reconcile').onclick=()=>action(async()=>{render(await request('/api/trading/reconcile'));await plot();});
  ...
  el('trading-close').onclick=()=>action(async()=>{render(await request('/api/trading/close',{reason:'用户请求全部平仓'}));});
  el('chart-window').onchange=()=>{resetChartZoom=true;action(plot);};
  el('chart-latest').onclick=()=>{resetChartZoom=true;action(plot);};
  window.addEventListener('resize',()=>chart?.resize());
  setTimeout(()=>status().catch(error=>text('trading-result',error.message,true)),400);
  timer=setInterval(()=>{if(!busy)status().catch(error=>text('trading-result',error.message,true));},3000);
})();
