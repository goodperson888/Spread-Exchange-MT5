...
(() => {
  ...
  const eventLabels={boot:'程序启动',start:'启动自动开仓',pause:'暂停自动开仓',entry_preflight_blocked:'开仓预检暂未通过',entry_skipped:'跳过本次开仓',group_opening:'准备开仓',group_opened:'双边开仓完成',group_closed:'交易组已结束',order_intent:'准备发送订单',order_result:'订单回报',order_reconciled:'订单对账完成',entry_recovered:'延迟回报已确认，双边开仓完成',order_reconcile_retry:'订单对账重试',order_reconcile_exhausted:'快速确认结束，继续低频对账',close_requested:'请求平仓',close_attempt:'尝试平仓',exit_signal:'触发退出条件',entry_threshold_applied:'开仓阈值已更新',reconciled:'持仓对账通过',paper_reconciled:'纸面持仓对账通过',auto_reconciled_flat:'空仓自动对账通过',costs_verified:'费用已核验',cost_verification_pending:'等待费用核验',adoption_management:'已有仓位管理状态变更',adoption_closing_costs_checked:'接管仓位平仓费用已核验'};
  const eventFields={request_ms:'下单回报耗时（毫秒）',query_ms:'订单查询耗时（毫秒）',threshold:'开仓阈值',reason:'原因',state:'状态',group:'交易组',id:'订单标识',leg:'平台',action:'操作',qty:'数量',price:'价格',status:'回报状态',error:'说明',result:'回报',mode:'模式',entry:'开仓价差',value:'数值',attempt:'尝试次数',net:'净收益',net_estimate:'预估净收益',signal_spread:'触发价差',binance_qty:'币安数量',mt5_lots:'MT5手数',position_mode:'持仓模式',enabled:'启用',code:'原因代码',ticket:'订单号',submitted:'已发送',raw_status:'交易所状态',error_code:'错误码',fee:'手续费'};
  const eventValues={waiting:'等待检查',checking:'检查中',expired:'已过期',failed:'失败',ready:'通过',done:'已结束',pending:'待确认',unknown:'未知',binance:'币安',mt5:'MT5',open:'开仓',close:'平仓',paper:'纸面',live:'实盘',hedge:'双向持仓',one_way:'单向持仓'};
  function eventDetails(value,key='') {
    if(value&&typeof value==='object') return Object.entries(value).map(([k,v])=>`${eventFields[k]||k}：${eventDetails(v,k)}`).join('；');
    if(typeof value==='boolean')return value?'是':'否';
    return ['state','status','leg','action','mode','position_mode'].includes(key)?(eventValues[value]||String(value??'—')):String(value??'—');
  }
  function renderEvents(events) {
    const rows=[];
    for(const event of events.slice(0,100)) {
      const previous=rows[rows.length-1], signature=event.kind+JSON.stringify(event.data||{});
      if(previous&&previous.signature===signature&&Math.abs(previous.time-event.time)<15000) {previous.count++;continue;}
      rows.push({...event,signature,count:1});
    }
    const content=rows.map(x=>{
      const data=x.data||{};
      const detail=x.kind==='entry_preflight_blocked'
        ?`开仓阈值 ${data.threshold??'—'} USD/盎司；${data.reason||'旧日志未记录具体原因，请查看开仓阈值下方的预检状态'}；本次未下单，等待预检通过`
        :eventDetails(data);
      return `${dateTime(x.time)}  ${eventLabels[x.kind]||'技术事件（'+x.kind+'）'}${x.count>1?'（合并 '+x.count+' 条）':''}\n${detail}`;
    }).join('\n\n')||'暂无交易事件';
    const node=el('trade-events');
    if(node.textContent!==content) {
      const offset=node.scrollTop;node.textContent=content;node.scrollTop=offset;
    }
  }
  ...
  function groupRows(groups, orders) {
    ...
    const rows=groups.slice().reverse().map(g => {
      ...
      const abnormalClosed=g.status==='closed'&&/拒单|异常|敞口|部分/.test(String(g.reason||''));
      ...
      const feeDetail=exitFee>0?`<br><span class="muted" title="按当前币安 Ask × USDT/USD × 币安 taker 费率，加上 MT5 剩余手数 × 单边每手佣金估算">预计平仓费 -${exitFee.toFixed(2)} USD<br>币安 -${exitBinance.toFixed(2)} + MT5 -${exitMt5.toFixed(2)}</span>`:'';
      const stateLabel=abnormalClosed?'异常撤回已完成':states[g.status]||g.status;
      const pnlLabel=g.status==='closed'?(abnormalClosed?'异常撤回净结果':'平仓净收益'):'实时净收益';
      return `<tr><td><strong>#${escape(g.id)}</strong>${gridLabel}<br><span class="muted">${dateTime(g.opened_ms)}${g.closed_ms?'<br>→ '+dateTime(g.closed_ms):''}</span></td><td>${escape(stateLabel)}<br><span class="record-basis">${basis}</span>${g.execution_warning?'<br><span class="muted">'+escape(g.execution_warning)+'</span>':''}</td><td>${n(g.lots)} 手 / ${n(g.qty)} 盎司${entryLabel}<br><span class="muted">币安 ${amount('binance').toFixed(2)} USDT<br>MT5 ${amount('mt5').toFixed(2)} USD</span></td><td>毛收益 ${usd(v.gross)}<br><span title="已由成交回报或平台记录确认">已成交手续费 -${Number(v.fees||0).toFixed(2)} USD</span>${feeDetail}</td><td>${carryDetail}</td><td class="${Number(v.net)>=0?'pnl-positive':'pnl-negative'}"><strong>${usd(v.net)}</strong><br><span class="muted">${pnlLabel}</span></td><td>币安 ${n(remaining.binance||0)}<br>MT5 ${n(remaining.mt5||0)}${g.reason?'<br>'+escape(g.reason):''}</td><td>${action}</td></tr>`;
    }).join('');
  ...
  function orderRows(orders,groups) {
    ...
    const rows=orders.slice().reverse().slice(0,100).map(o=>{
      ...
      const timing=o.imported?'':Number.isFinite(r.fill_time_ms)?`<br><span class="muted">发送→回报 ${Math.max(0,r.fill_time_ms-o.created_ms)} ms${Number.isFinite(o.signal_time_ms)?'<br>采样→发送 '+Math.max(0,o.created_ms-o.signal_time_ms)+' ms':''}</span>`:Number.isFinite(r.reconcile_time_ms)?`<br><span class="muted">查询确认 ${Math.max(0,r.reconcile_time_ms-o.created_ms)} ms（非实际下单延迟）</span>`:r.status==='done'&&filled>0?'<br><span class="muted">成交回报耗时未记录</span>':'';
  ...
  function updateEntryLabel() {
    const effective=el('entry-effective'), value=effective.dataset.value;
    const pending=el('entry').value.trim();
    effective.textContent=value!==undefined&&value!==''
      ? `当前生效：${n(Number(value))}`+(pending!==''&&Number(pending)===Number(value)?'（已有交易组不变）':` · 待应用：${pending||'未填写'}`)
      : '当前生效：未连接';
  }
  ...
  function renderIncome(q) {
    ...
    el('pnl-stats').innerHTML=[['实时净收益',usd(sum(active,'net'))],['已平仓净收益',usd(sum(closed,'net'))],['累计资金费 + Swap（含已平仓）',usd(allCarry)],['手续费及预估平仓费',`-${allFees.toFixed(2)} USD`]].map(([label,value])=>`<div class="stat"><span>${label}</span><strong>${value}</strong></div>`).join('');
  ...
  function drawChart(rawSamples) {
    ...
    const effective=last?.connected?last.strategy_runtime?.entry_spread_usd:null;
    const threshold=Number.isFinite(effective)?effective:null;
  ...
  el('entry').addEventListener('input',updateEntryLabel);
